    ...
    if ($charge->amount_refunded == 0) {
        ...
        if ($conn->query($sqll) === TRUE) {
            ...
            if ($conn->query($deleteCart) === TRUE) {
                header('Location: http://34.214.5.64/instacraft/cus-my-orders');
            }
        } else {
        }
...
<form action="product_form.php" method="post">
    <script
    	...
    	data-amount="<?=($_POST['payable_amount']*100)?>"
    	data-locale="us-en"
    	data-name="Product Plan Demo"
    	data-description="Product description (<?=($_POST['payable_amount'])?>)"
