...
function buildMessageHTML(messageText, messageSenderId, messageDateSent, attachmentFileId, messageId, status){
  ...
  var messageHtml =
			'<div class="list-group-item" id="'+messageId+'" onclick="clickToAddMsg('+"'"+messageId+"'"+')">'+
				'<time datetime="'+messageDateSent+ '" class="pull-right toshow">'
...
// build html for messages
function buildMessageHTMLCustom(messageText, messageSenderId, messageDateSent, attachmentFileId, messageId, status,fullName){
  var messageAttach;
//  alert(messageSenderId);
  ...
  var messageHtml =
			'<div class="list-group-item" id="'+messageId+'" onclick="clickToAddMsg('+"'"+messageId+"'"+')">'+
				'<time datetime="'+messageDateSent+ '" class="pull-right time-toshow">'
					+jQuery.timeago(messageDateSent)+
				'</time>'+

				'<h4 class="list-group-item-heading">'+messageSenderId+'</h4>'+
				'<p class="list-group-item-text '+messageSenderId+'">'+
					messageTextHtml +
				'</p>'
				+delivered+read+
			'</div>';
...
function buildDialogHtml(dialogId, dialogUnreadMessagesCount, dialogIcon, dialogName, dialogLastMessage) {
  ...
  var dialogHtml =
      '<a class="list-group-item chat-adm inactive" id='+'"'+dialogId+'"'+' onclick="triggerDialog('+"'"+dialogId+"'"+')">'+
                   (dialogUnreadMessagesCount === 0 ? UnreadMessagesCountHide : UnreadMessagesCountShow)+
        '<h4 class="list-group-item-heading">'+ dialogIcon+'&nbsp;&nbsp;&nbsp;' +
            '<span class="chat-with">'+dialogName+'</span>' +
        '</h4>'+
