...
//QB.users.get({'tags':data.room,'full_name':data.username,'per_page': 100}, function(err, result){
//    //alert(JSON.stringify(err));
//       _.each(result.items, function(item) {
//           //alert(JSON.stringify(item));
//                        //users.push(item.user);
//if(item.user.full_name=='support' && item.user.user_tags=='biggdeal') {
//                      var id=item.user.id;
//                      //alert(id);  
//                    
//var user_id =id;
//// alert(user_id);
//QB.createSession(function(err123, res123){ 
//var params = {'email':"ceo@bigdeals.in", 'password':"techahead"};
// 
//QB.login(params, function(err, user){
//    //alert(JSON.stringify(err));
//  QB.users.delete(user_id, function(err, user111){
//    //alert(JSON.stringify(err));
//  if (user111) {
//   //aler('success');
//  } else  {
    ...
    webapp.helpers.renderUsers = function() {
        return new Promise(function(resolve, reject) {
            var tpl = _.template( $('#user_tpl').html() ),
                usersHTML = '',
                users = [];
            QB.users.get({'tags':[webapp.caller.user_tags], 'per_page': 100}, function(err, result){
                if (err) {
                    reject(err);
                } else {
                    _.each(result.items, function(item) {
