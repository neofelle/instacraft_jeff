...
function setupMsgScrollHandler() {
  ...
  msgList.scroll(function() {
    if (msgWindow.scrollTop() == msgWindow.height() - msgList.height()){
     ...
     // alert(JSON.stringify(currentDialog));
    ...
    QB.chat.message.list(params, function(err, messages) {
      if (messages) {
        ...
        messages.items.forEach(function(item, i, arr) {
          ...
          var fullName=getUserFirstNameById(messageSenderId);
          ...
          //var messageHtml = buildMessageHTML(messageText, messageSenderLogin, messageDateSent, messageAttachmentFileId, messageId,'',fullName);
          var messageHtml = buildMessageHTMLCustom(messageText, fullName, messageDateSent, messageAttachmentFileId, messageId);
...
function showMessage(userId, msg, attachmentFileId) {
  ...
  var fullName=getUserFirstNameById(userId); 
  
//  var messageHtml = buildMessageHTML(msg.body, userLogin, new Date(), attachmentFileId, msg.id);
var messageHtml = buildMessageHTMLCustom(msg.body, fullName, new Date(), attachmentFileId, msg.id);
