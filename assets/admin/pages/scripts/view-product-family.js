...
$( "#familyname" ).on('keyup change paste',function(){
    var family  = $("#familyname").val();
    var urlpart = baseUrl+"/check-product-family";
    datapart = {familyname: family};
    if(family != ''){
        $.ajax({
            type: "POST",
            url: urlpart,
            ...
            complete: function (response) {
                ...
                if(response.status == 200){
                    ...
                    if(res == 'true'){
                        catCheckVal = 'false';
                        $('#nametesting').html('old name');
                        $('#fmailychk').attr('class','fa fa-close');
                    }else{
                        catCheckVal = 'true';
                        $('#fmailychk').attr('class','fa fa-check-circle');
                        $('#nametesting').html('new name');
                    }
                }
...
//-- Created By N.K.
    ...
    $("#addProductFamily").submit(function(event){ 
        ...
        if(familyname == '' || familyname == undefined){
            ...
            washFeedError('familyname-error');
            error['familyname'] = false;
        }
       
        if(error['familyname'] == false){
            return true;
        }else{
            return false;
        }
