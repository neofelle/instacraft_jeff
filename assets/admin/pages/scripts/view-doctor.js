jQuery(document).ready(function () {
        ...
        //-- Ajax for Particular Prescription Details 
        ...
        {   
            ...
            $.ajax({
                ...
                complete: function(result) 
                {
                    ...
                    $('#appointmentUserInfoModal').show();
                    $('#appointData').html(result.responseText);
                }
            });
        });
        
        //-- Ajax for Particular Prescription Details 
        $(".prescriptionInfo").click(function() 
        {   
            var url = $(this).attr('data-href');
            var val = $(this).attr('data-val');
            $.ajax({
                ...
                {
                    //console.log(result.responseText);
                    $('#prescriptionUserInfoModal').show();
                    $('#prescriptionData').html(result.responseText);
                } 
            });
        });
        $(document).on("click","#closePop,#closePrescription,#closeAppointment",function() {//disable both popup
            $('#appointmentUserInfoModal').hide();//hide deactive driver popup
            $('#prescriptionUserInfoModal').hide();// hide activate driver popup
        });
        ...
        $(document).on("click","#closePopForDel,#closePop",function() {//disable both popup
    ...
    function formSearchByDate(eleVal){
        if(eleVal != '')$('<form action=""><input type="hidden" value="'+eleVal+'" name="date" id="date" /></form>').appendTo('body').submit();
    }
...
//-- Created By N.K.
    ...
    //--- Validate Update Form Submission 
    $("#updateDoctor").submit(function(event){ 
        ...
        if(profile_pic != '' && !validate_uploadfile_format(profile_pic,'image')){
        ...
        if(file_1 != '' && file_1 != undefined && !validate_uploadfile_format(file_1,'imgdocs')){
        ...
        if(file_2 != '' && file_2 != undefined && !validate_uploadfile_format(file_2,'imgdocs')){
        ...
        if(file_3 == '' && file_3 == undefined && !validate_uploadfile_format(file_3,'imgdocs')){
