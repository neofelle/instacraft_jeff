...
function onJoined(room) {
    $("#joinBtn").text("End Call").off('click').click(function(){
...
function onLeft() {
    ...
    $("#joinBtn").text("Call").off('click').click(function(){
        if (validateForm()) {
            $(this).prop('disabled', true);
            muteConnectInputs();
            start();
...
function joinRoom() {
    connection.join({name: getRoomName()}).on(ROOM_EVENT.STATE, function(room){
        ...
        if (participants.length > 0) {
            ...
            $("#joinBtn").addClass('doctor');
            // change call_status in appointment_detail table when doctor accept the call
            var appointment_id  =   $('#appoint_id').val();
...
function removeParticipant(participant) {
    $("[id$=Name]").each(function(index,value) {
        ...
        if ($(value).text() == participant.name()) {
            $(value).text('NONE');
            if(doctor_name == participant.name()){
                appointment_id  =   $('#appoint_id').val();
                redirect_url    =    siteurl+'cus-home';
...
function setInviteAddress(name) {
    ...
    $('#inviteAddress').text(window.location.href.split("?")[0] + "?roomName="+name+"&appointment_id="+appointment_id);
    apt_id  =   appointment_id; 
    if(appointment_id != '' && appointment_id != undefined){
        $.ajax({
            type: 'POST',
            data: {videoRoomId: name,appointment_id: appointment_id},
            url: siteurl + 'save-video-room',
            success: function () {
                //alert('video room saved against appointment_id '+appointment_id);
            }
        });
    }
}
