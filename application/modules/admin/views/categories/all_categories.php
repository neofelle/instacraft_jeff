    ...
    <div class="page-content" style="min-height:895px"> 
        ...
        <h3 class="page-title ">All Categories </h3>
            <form action='' id="searchData" method="post" >  
               <input type="text" class="search-custom" style="width:32%;" value="<?php echo $this->input->post('searchText')!= '' ? $this->input->post('searchText'):'';?>" name="searchText"  placeholder="Search by Category Name">
<!--                <select class="form-control selectSearch" id="doctorStatus" name="doctorStatus" onChange="formsubmit(this.form.id)">
            ...
            <div class="form-group pull-right">
                <a class="btn red reset p-xl-pad" href="add-category" id="add_category"><i class="fa fa-plus" style="color:white;"></i> Add New Category</a>
            </div>
                    ...
                    <table class="table table-bordered table-striped table-condensed flip-content table-hover" >
                        <thead class="flip-content">
                            <tr>
                                ...
                                <th style="width:250px;"> Sub-Category Name </th>
                                <th class="numeric" style="width:140px;"> Created Date </th>
                                <th class="numeric" style="width:65px;"> Total Products </th>
                        ...
                        <?php
                         if (count($result) > 0) {
                            ...
                            foreach ($result as $key => $list) {
                                ...
                                $subcat_list = '';
                                //$subcat_list = $list['catname'].'('.$list['items']['items_count'].')<br style="border-bottom:1px dashed cyan;" />';
                                if(count($list['subcategories']) > 0){
                                    $ssp = 1;
                                    foreach ($list['subcategories'] AS $keylist => $sublist){
                                        $subcat_list .= '<b title="Sub-category id - '.$sublist['subcatid'].'">'.$sublist['subcatname'].'</b>';
                                        //$subcat_list .= $sublist['subcatname'].'('.$sublist['items']['items_count'].')<br/>';
                                        if($ssp < count($list['subcategories'])) $subcat_list .= '<hr class="subCatHr" >';
                                        $ssp++;
                                    }
                                }
                        ...
                        ?>
                                    ...
                                    <tr id="row">
                                        <td> <?php echo $i + 1; ?></td>
                                        <td class="numeric editable"><b title="<?php echo  'Category id - '.$list['catid']; ?>"><?php echo ucfirst($list['catname']); ?></b></td>
                                        <td class="numeric editable"><?php echo ucfirst($subcat_list); ?></td>
                                        <td class="numeric editable"><?php echo $cdate->format('d-M-Y'); ?> </td>
                                        <td class="numeric editable"><?php echo $list['all_items_count'] ? $list['all_items_count'] : 0 ;   ?></td>
                                        <!--
                                        <td class="action">
                                            <span class="no_wrap">
                                                <a title="" id ="viewItem" href="category-details/<?php echo $list['itemid']; ?>" class="btn btn-default btn-xs"> <i class="fa fa-eye" style="color:red;"></i> </a>
                                            </span>
                                        </td>
                                        -->
                                    </tr>                                
